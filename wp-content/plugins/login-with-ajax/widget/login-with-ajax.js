jQuery(document).ready(function($){
		...
		$.post(url,postData,function(data){
			...
			if(data.result===true)
			{
				if(data.widget!=null)
				{
					...
					if(data.redirect==null){window.location.reload();}
					else{window.location=data.redirect;}}}},"json");});
	
	
	$('#LoginWithAjax_Remember').submit(function(event){event.preventDefault();$('<div class="LoginWithAjax_Loading" id="LoginWithAjax_Loading"></div>').prependTo('#LoginWithAjax');var url=$('#LoginWithAjax_Remember').attr('action');var postData=getPostData('#LoginWithAjax_Remember *[name]');$.post(url,postData,function(data){lwaAjax(data,'LoginWithAjax_Status','#login-with-ajax');},"json");});
	...
	$('#LoginWithAjax_Register form').submit(function(event){
			...
			var postData=getPostData('#LoginWithAjax_Register form *[name]'); console.log(postData);
			
			$.post(url,postData,function(data){lwaAjax(data,'LoginWithAjax_Register_Status','#LoginWithAjax_Register');},"json");
			});
		...
		function lwaAjax(data,statusElement,prependTo){
			...
			if(data.result===true||data.result===false)
			{
				if(data.result===true)
				{
					...
					if($('#'+statusElement).length>0){$('#'+statusElement).attr('class','invalid').html(data.error);}
					else{$('<span id="'+statusElement+'" class="invalid">'+data.error+'</span>').prependTo(prependTo);}
					
					$('#'+statusElement).click(function(event){
						event.preventDefault();$('#LoginWithAjax_Remember').show('slow');});
				}
			}
			else{
				if($('#'+statusElement).length>0)
