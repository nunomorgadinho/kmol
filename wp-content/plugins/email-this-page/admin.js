...
jQuery(document).ready(function() {
	...
	jQuery('#colorSelector').ColorPicker({
		...
		onChange: function (hsb, hex, rgb) {
			...
			jQuery('.instaemail-text').css('color','#' + hex)
		}
	});
	...
	jQuery('#text_size').change(function(){
		...
		jQuery('.instaemail-text').css('font-size',parseInt(size));
	}).change();
	...
	function pf_custom_text_change(){
		jQuery('.button_preview span').text( jQuery('#custom_text').val() );
	}
	
	jQuery('.instaemail-text').css('color', jQuery('#text_color').val() );
});
